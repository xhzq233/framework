...
class SwiperController extends ChangeNotifier {
  ...
  _SwiperState get _state {
    assert(_swiperState != null, "SwiperController cannot be accessed before a Swiper is built with it");
    return _swiperState!;
  }
...
class _SwiperIndicator implements SwiperIndicator {
  ...
  Widget build(BuildContext context, int index, int itemCount) {
    ...
    var children = List.generate(itemCount, (int index_) {
...
class _SwiperState extends State<Swiper> with SingleTickerProviderStateMixin<Swiper> {
  ...
  Widget build(BuildContext context) {
    ...
    children.add(PageView.builder(
      ...
      itemCount: widget.circular && widget.childCount != 1 ? null : widget.childCount,
