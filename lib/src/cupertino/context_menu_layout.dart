...
bool _menuPositionIsBelow(Rect childRect, EdgeInsets padding, Size screenSize) {
  ...
  final double spaceBelow =
      screenSize.height - padding.bottom - childRect.bottom;
  final double spaceAbove = childRect.top - padding.top;
  return spaceBelow > spaceAbove;
}

(double, double) _getMenuOrigin(
    Rect childRect, Size menuSize, EdgeInsets padding, Size screenSize) {
...
class _ContextMenuLayoutDelegate extends BoxyDelegate {
  ...
  Size layout() {
    ...
    // 计算menu大小
    final menuSize =
        menu.layout(BoxConstraints(maxWidth: maxWidth, maxHeight: maxHeight));
    Size childSize = child
        .layout(BoxConstraints(maxWidth: childWidth, maxHeight: childHeight));

    final Matrix4 childTransform;

    // 拉伸child大小
    final childLimitWidth =
        math.min(maxWidth - 36, childGlobalRect.width * maxScale);
    final childLimitHeight =
        math.min(maxHeight, childGlobalRect.height * maxScale);
    final childScale = math.min(
        childLimitWidth / childSize.width, childLimitHeight / childSize.height);
    childSize *= childScale;

    /// 1. 超出屏幕，缩放child到合适大小
    /// 2. 被屏幕边缘遮挡，不需要缩放，origin延用之前的rect中心，clamp到屏幕边缘
    /// 3. 正常情况，没有超出就延用之前的rect的y坐标，x居中
    final bool overflow = (childSize.height +
            menuSize.height +
            _ContextMenuRouteStaticState._kPadding) >
        maxHeight;
    if (overflow) {
      final double scale = (maxHeight -
              menuSize.height -
              _ContextMenuRouteStaticState._kPadding) /
          childSize.height;
    ...
    (menuOriginX, menuOriginY) =
        _getMenuOrigin(childOrigin & childSize, menuSize, padding, screenSize);
    final menuTransform =
        Matrix4.translationValues(menuOriginX, menuOriginY, 0);
